...
- Suggest next steps"""
    ...
    async def export_findings(self, format: str = "json") -> str:
        ...
        if format == "json":
            ...
            # Get the final report summary from manager
            report_summary = ""
            if hasattr(self.manager, 'all_findings') and self.manager.all_findings:
                # Try to get the synthesis
                try:
            ...
            lines = [
                f"# Research Report: {self.current_session.goal}",
                "",
                "---",
                "",
                f"**Session ID:** {self.current_session.id}",
                f"**Started:** {self.current_session.started_at}",
                f"**Ended:** {self.current_session.ended_at or 'In Progress'}",
            ...
            for ftype, flist in sorted(by_type.items()):
                lines.append(f"- **{ftype.upper()}**: {len(flist)} findings")
            lines.append("")

            # Top findings by confidence
            lines.append("## Key Findings (Top 10 by Confidence)")
            lines.append("")
            top_findings = sorted(findings, key=lambda f: f.confidence, reverse=True)[:10]
            for i, f in enumerate(top_findings, 1):
                lines.append(f"### {i}. [{f.finding_type.value.upper()}] (Confidence: {f.confidence:.0%})")
                lines.append("")
                ...
                if f.search_query:
                    lines.append(f"**Search Query:** `{f.search_query}`")
                lines.append("")

            # All findings by type
            lines.append("## All Findings by Type")
            lines.append("")

            for ftype in ["fact", "insight", "source", "connection", "question", "contradiction"]:
                if ftype in by_type:
                    lines.append(f"### {ftype.upper()}S ({len(by_type[ftype])})")
                    lines.append("")
            ...
            lines.append("---")
            lines.append("")
            lines.append(f"*Generated by Claude Deep Researcher on {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}*")
